...
import com.exam.repos.Non_IT_QuestionRepo;
@Service
public class IT_QuestionService implements StudentITQuestion {
	@Autowired
	IT_QuestionRepo qreRepo;
	@Autowired
	Non_IT_QuestionRepo nqreRepo;
	
	static final Logger logger = Logger.getLogger(IT_QuestionService.class);
	...
	public ArrayList<IT_Questions> getAllquestion(String sesn,String aptitype) {
		...
		Non_IT_Questions nques=new Non_IT_Questions();
...
//			String questionsql = null;
			try {
				...
				if (aptitype.equals("it")) {	
					...
					setList=nqreRepo.getNonitSetnum();
				}
				...
				if (aptitype.equals("it")) {
					...
					nques=nqreRepo.getAllnonitQuestion(selected_set_no);
					 ques.setError(0);
					 Questionlist.add(ques);
				}
